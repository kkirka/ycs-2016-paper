...
\paragraph{II. Updating states of nodes in parallel.} Another way to reduce an execution time of modeling is to parallelize calculations of nodes' states at a particular iteration. In this study, we propose a GPGPU algorithm which speeds up sequential heuristics of type (I). As it does not influence the logic of a search, this algorithm can be used in a combination with any sequential heuristic. The main idea of the algorithm is to update states of different nodes in a network in parallel on distinct Graphic Processor Units (GPUs). More formally,... 

	[To do. A description of parallel algorithm]
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\textwidth]{plots/evolution}
		\caption{Evolution of mean network connectivity}
	\end{figure}
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\textwidth]{plots/n_rew}
		\caption{Evolution of mean connectivity for different number of nodes rewired each epoch; $N=1000$}
	\end{figure}
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\textwidth]{plots/attractors}
